...
mod tests {
    ...
    use crate::data::batch::cifar100::{Cifar100Batch, Cifar100Batcher};
    use crate::data::batch::imagenet1k::{ImageNet1kBatch, ImageNet1kBatcher};
    use crate::data::batch::mnist::{MnistBatch, MnistBatcher};
    ...
    fn test_imagenet1k() {
        ...
        let imagenet1k_batcher = ImageNet1kBatcher::new();

        let imagenet1k_train_dl =
            StreamingDataLoaderBuilder::<B, ImageNet1kBatch<B>>::new(imagenet1k_batcher)
                .with_batch_size(batch_size)
                .with_shuffle(shuffle_seed)
                //.with_batch_mapper(ImageNet1kMapper::decoder())
                .with_device(device)
                .build(imagenet1k_ds.train());
